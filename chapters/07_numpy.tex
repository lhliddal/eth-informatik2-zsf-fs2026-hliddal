\section{Numpy}\label{sec:07-numpy}


% -----------------------------
\subsection{Array-Grundlagen}\label{sec:07-array-grundlagen}
\ZSFindex{NumPy}
\subsubsection{Array erzeugen / Konstruktoren}\label{sec:07-array-erzeugen}
\ZSFindex{Array}
\begin{codebox}
...
\subsubsection{Typumwandlung}\label{sec:07-typumwandlung}
\begin{codebox}
\begin{lstlisting}[style=CodeExpert]
a.astype(np.float32)           # in float32 umwandeln (kopiert)
# kein Copy falls dtype schon passt
a.astype("int32", copy=False)
a.tolist()                     # zurück zur Python-Liste
\end{lstlisting}
\end{codebox}

\subsection{Zugriff und Shape-Operationen}\label{sec:07-zugriff-und-shapes}
\subsubsection{Indexing, Slicing, Maskierung}\label{sec:07-indexing-slicing-maskierung}
\ZSFindex{Boolean Mask}
\begin{codebox}
\begin{lstlisting}[style=CodeExpert]
...
\begin{contentbox}[2D-Slicing]
\begin{center}
\includegraphics[width=\ZSFgraphicswidth]{Images/numpy-2d-array-slicing-examples.pdf}
\end{center}
\end{contentbox}

\begin{contentbox}[Boolean Mask]
\begin{center}
\includegraphics[width=\ZSFgraphicswidth]{Images/numpy-boolean-mask-filtering.pdf}
\end{center}
\end{contentbox}


\subsubsection{Broadcasting \& Form ändern}\label{sec:07-broadcasting-und-shapes}
\ZSFindex{Broadcasting}
\ZSFindex{reshape}
\begin{codebox}
\begin{lstlisting}[style=CodeExpert]
...
np.expand_dims(a, axis=0)   # neue Achse einfügen
np.squeeze(a)               # Achsen der Grösse 1 entfernen
np.reshape(a, (2, -1))      # Form ändern (-1 = auto)
np.tile(a, (2, 3))          # Muster 2x vertikal, 3x horiz.
\end{lstlisting}
\end{codebox}


\subsubsection{Stacking \& Splitting (Zusammenfügen / Aufteilen)}\label{sec:07-stacking-und-splitting}
\begin{codebox}
\begin{lstlisting}[style=CodeExpert]
# Kombinieren von Arrays
np.concatenate((a, b), axis=0)  # entlang bestehender Achse
np.stack([a, b], axis=0)        # neue Achse einfügen
...
np.vsplit(a, 2)                 # vertikal in 2 Hälften teilen
np.hsplit(a, 2)  # horizontal in 2 Hälften teilen
\end{lstlisting}
\end{codebox}


\subsection{Berechnen und Zusammenfassen}\label{sec:07-berechnen-und-zusammenfassen}
\subsubsection{Elementweise Mathematik}\label{sec:07-mathematik}
\begin{codebox}
\begin{lstlisting}[style=CodeExpert]
...
\subsubsection{Aggregate und Statistik}\label{sec:07-aggregate-und-statistik}
\begin{codebox}
\begin{lstlisting}[style=CodeExpert]
...
np.min(a), np.max(a)        # kleinster, grösster Wert
