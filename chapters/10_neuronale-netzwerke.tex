...
\ZSFFact{$f$}{Aktivierungsfunktion (siehe unten)}
\ZSFFact{Training}{Gewichtungen $w_{ki}$ und Bias $b_k$ werden im Training angepasst}
\end{factlist}
\end{contentbox}

\begin{contentbox}[Aktivierungsfunktionen]
\begin{factlist}
\ZSFFact{ReLU}{$f(x) = \max(0, x)$ — oft in versteckten Schichten}
\ZSFFact{Sigmoid}{$\sigma(x) = \frac{1}{1 + e^{-x}}$ — Ausgabe bei binärer Klassifikation}
\end{factlist}
\end{contentbox}

\subsection{Softmax (Ausgabeschicht)}\label{sec:10-softmax-schicht-ausgabe}
\begin{contentbox}
\begin{factlist}
...
\subsection{CNN (Convolutional Neural Network)}\label{sec:10-convolutional-neural-network-cnn}
\begin{contentbox}
...
\begin{procedure}[CNN: Schritt für Schritt]
