...
for sub in "${IMAGE_DIRS[@]}"; do
  GRAPHICS_DIR="${PROJECT_ROOT}/${sub}"
  if [ ! -d "${GRAPHICS_DIR}" ]; then
    echo "⏭  ${sub}/ existiert nicht — übersprungen."
    continue
  fi
  BACKUP_DIR="${GRAPHICS_DIR}/_originals"
  if [ ! -d "${BACKUP_DIR}" ]; then
    mkdir -p "${BACKUP_DIR}"
    echo "✓ Backup-Verzeichnis erstellt: ${sub}/_originals/"
  fi
  echo "— ${sub}/ —"
...
while IFS= read -r -d '' img; do
  ...
  echo "Neue Gesamtgrösse ${sub}/: ${size_total_after}"
  echo ""
done

echo "=== Fertig ==="
echo "Total: ${total} Bilder | Optimiert: ${optimized} | Übersprungen: ${skipped}"
echo "Originale gesichert in: <dir>/_originals/"
