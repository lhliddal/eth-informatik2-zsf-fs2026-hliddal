...
\subsection{DataFrame erzeugen}\label{sec:07-dataframe-erzeugen}
\begin{contentbox}
...
\subsection{Zeilen/Spalten auswählen (loc / iloc)}\label{sec:07-auswaehlen}
\begin{contentbox}
...
\subsection{Zeilen filtern (Bedingung / boolean mask)}\label{sec:07-filtern-von-zeilen}
\begin{contentbox}
\CodeLine{data[data["Age"] > 30]}[alle Personen über 30]\\
\CodeLine{data["Name"][data["Age"] > 30]}[nur Namen dieser Personen]
\end{contentbox}

\subsection{Index \& Spalten umbenennen}\label{sec:07-spalten-umbenennen}
\begin{contentbox}
...
\subsection{Löschen \& fehlende Werte (NaN)}\label{sec:07-zeilen-und-spalten-loeschen}
\begin{contentbox}
