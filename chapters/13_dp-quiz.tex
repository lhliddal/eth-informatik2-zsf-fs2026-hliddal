% ============================================================
% EIGENSTÄNDIGER TEIL — DP-QUIZ
% Dieses Kapitel ist bewusst unabhängig vom restlichen Skript:
%   - kein anderes Kapitel referenziert ein sec:13-Label
%   - der Quiz selbst referenziert keine externen Labels/Inhalte
% => komplett entfernbar durch Löschen der Zeile
%      \input{chapters/13_dp-quiz}
%    in main.tex (kein weiterer Eingriff nötig).
% Im Kapitelbalken als "eigenständiger Teil" markiert (ZSFTitleTag).
% Layout-Markup-Pass: hier nur minimal angefasst (s. _scratch/layout-markup-pass.md).
% ============================================================
\section{DP-Quiz — Übungsfragen\ZSFTitleTag{eigenständiger Teil}}\label{sec:13-dp-quiz}

\subsection{Multiple-Choice: Konzept-Fragen}\label{sec:13-welche-aussage-ist-korrekt}
...
\begin{contentbox}
Die $n$-te Fibonacci-Zahl $F(n)$ ist definiert als $F(n) = F(n-1) + F(n-2)$ mit den Basisfällen $F(1) = 1$ und $F(2) = 2$. Betrachte einen \ZSFkeyword{Top-down}-DP-Algorithmus, der $n$ als Eingabe nimmt und die $n$-te Fibonacci-Zahl berechnet. Was beschreibt die Notwendigkeit von Memoization am besten?
\begin{factlist}
...
\subsection{Wahr/Falsch: Aussagen prüfen}\label{sec:13-wahr-oder-falsch}
\begin{contentbox}
...
\begin{codebox}[Laufzeit empirisch schätzen][Python]
